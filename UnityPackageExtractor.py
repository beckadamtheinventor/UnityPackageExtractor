...
def ExtractPackage(fname, dname):
	...
	with TemporaryDirectory(prefix="PyUnityPackageExtractor") as d:
		...
		for root,dirs,files in os.walk(d):
			for dn in dirs:
				with open(os.path.join(root, dn, "pathname"), "rb") as f:
					pathname = f.read()
				path_o = []
				for c in pathname:
					try:
						...
						pass
				pathname = "".join(path_o)
				try:
					name = os.path.join(dname, os.path.dirname(pathname))
					os.makedirs(name)
					print(name + "/")
				except:
					pass
				try:
					name = os.path.join(dname, pathname)
					shutil.copyfile(os.path.join(root, dn, "asset"), name)
					print(name)
				except:
					pass
...
if __name__=='__main__':
	...
	if len(sys.argv) < 2:
		if sg is None:
			console = True
			print("Missing PySimpleGui! Cannot start user interface.\nDefaulting to console input. Press control+C to exit.\n")
			f = input("Type or paste path to Unity package to extract: ").strip("\"")
			if not len(f):
				exit(0)
		else:
	...
	if o is None:
		o = os.path.join(os.path.dirname(f), f.rsplit(".", maxsplit=1)[0])
